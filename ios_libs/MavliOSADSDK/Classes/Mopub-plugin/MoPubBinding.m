                         ...
                         MoPubBackgroundEventCallback bgEventCallback)
{
    ...
    //[[NSNotificationCenter defaultCenter] postNotificationName:@"openTestModeNotification" object:nil];
    ...
    NSMutableDictionary *smaatoConfig = [NSMutableDictionary new];
        [smaatoConfig setObject:@"1100042676" forKey:@"publisherId"];
        [smaatoConfig setObject:@(YES) forKey:@"httpsOnly"];
        [smaatoConfig setObject:@(NO) forKey:@"loggingDisabled"];
    config.additionalNetworks = moPubExtractNetworkClasses("SMAMoPubBaseAdapterConfiguration");
    [config setNetworkConfiguration:smaatoConfig forMediationAdapter:@"SMAMoPubBaseAdapterConfiguration"];
    NSDictionary * ironSourceConfig = @{@"applicationKey": @"c1b94935"};
    [config setNetworkConfiguration:ironSourceConfig forMediationAdapter:@"IronSourceAdapterConfiguration"];
    ...
    [[MoPub sharedInstance] initializeSdkWithConfiguration:config completion:^{
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:@"OnMopubInitComplete" object:nil];
    }];
}
...
void _moPubSetBannerPosition(int x, int y, int w, int h)
{
    [[MoPubManager sharedManager] setBannerPositionWithPointX:x PointY:y SizeW:w SizeH:h];
}
...
void _moPubRefreshBannerKeywords(const char* adUnitId, const char* keywords, const char* userDataKeywords)
{
    [[MoPubManager managerForAdunit:GetStringParam(adUnitId)] refreshBannerKeywords:GetNullableStringParam(keywords)
                                                       userDataKeywords:GetNullableStringParam(userDataKeywords)];
}
...
void _moPubRequestNativeAd(int bannerPosition, const char* adUnitId)
{
    MoPubAdPosition position = (MoPubAdPosition)bannerPosition;
    
    [[MoPubManager managerForAdunit:GetStringParam(adUnitId)] requestNativeAd:position];
}

void _moPubSetNativePosition(int x, int y, int w, int h)
{
    [[MoPubManager sharedManager] setNativePositionWithPointX:x PointY:y SizeW:w SizeH:h];
}

void _moPubShowNatived(const char* adUnitId)
{
    [[MoPubManager managerForAdunit:GetStringParam(adUnitId)] showNatived];
}

void _moPubHideNatived(const char* adUnitId)
{
    [[MoPubManager managerForAdunit:GetStringParam(adUnitId)] hideNatived];
}

void _moPubDestoryNatived(const char* adUnitId)
{
    [[MoPubManager managerForAdunit:GetStringParam(adUnitId)] destoryNatived];
}

bool _moPubIsNativedAdloaded(const char* adUnitId)
{
    MoPubManager* mgr = [MoPubManager managerForAdunit:GetStringParam(adUnitId)];
    return mgr != nil && [mgr isNativedAdloaded];
}
